...
CMS 是高信息密度的内容工作台，不是博客首页的缩小版，也不是营销式统计卡片墙。桌面端采用稳定的“分组侧栏 + 页面上下文顶栏 + 主工作区”，编辑器再增加右侧检查器；移动端将侧栏收为焦点受控抽屉，数据表切换为语义化内容行。侧栏、顶栏、编辑器检查器与表单组用 surface 层级组织，危险操作仅使用克制的红色文本。

工作台参考 V3 Admin Vite 的响应式壳层、Pure Admin Thin 的 Element Plus 信息密度，以及 Vben 的页面/搜索表格职责划分，但不复制模板源码或导入其 Pinia、Axios、动态权限、Mock、国际化、多布局和图表体系。Element Plus 负责表单、表格、抽屉、上传、确认与反馈原语；品牌层级、数据状态、工作区结构、主题 token 与响应式布局由本设计系统控制。

CMS 的通用组件职责固定如下：

- `AdminShell`：侧栏、顶栏、跳到主内容、公开博客入口、快捷新建、主题切换、owner 菜单和移动端焦点管理。
- `AdminPage`：页面标题、简短说明、主操作、工具区、内容与页脚提示；页面标题使用紧凑 `title-md`，不使用公开首页的展示字号。
- `DataToolbar`：搜索、筛选、排序、结果计数、清除和刷新；控件顺序在各列表保持一致。
- `AsyncState`：分别表达首次加载、保留数据刷新、请求错误并重试、数据库真正为空和筛选无结果。
- `PaginationBar` 与 `StatusBadge`：统一分页触达尺寸与文章状态语言；状态色只使用 accent、neutral 与 danger 语义，不建立彩虹标签体系。

CMS 不使用 Tailwind。当前没有真实访问趋势数据或多人协作能力，因此不引入 ECharts、`vue-echarts` 或 Yjs；概览只能展示文章状态、分类、标签、媒体和最近修改等真实库存数据。
...
宽屏使用品牌说明与登录纸面并置，窄屏收为单列。必须明确“仅限站点管理员”，只提供邮箱、密码、登录与主题切换，不出现注册、社交登录或营销数据。错误提示占用稳定空间，提交中禁用重复操作。

### CMS 概览

顶部用一句问候、当前内容状态和“新建文章”建立方向；下方使用一条紧凑统计带展示草稿、已发布、已归档、分类、标签与媒体真实数量。最近修改和待完成草稿是主内容，不绘制没有数据来源的趋势图。快捷操作以短列表呈现，不制造等权卡片墙。

### CMS 文章列表

工具区依次为标题搜索、状态、分类、排序、清除与刷新，并展示精确结果数。桌面表格把标题/摘要作为主列，状态、分类、更新时间和操作作为辅助列；移动端每篇文章是一行可点击内容块，不能把桌面表格简单压缩或只提供横向滚动。行操作进入独立 loading，发布、转草稿与归档互斥。

### CMS 编辑器

编辑页顶部固定显示返回路径、文章状态、未保存提示、最近保存时间和主要操作。主体以标题、Slug、摘要和 CodeMirror 为主工作面；右侧检查器组织分类、标签、封面、正文统计和发布检查。封面显示真实预览；已发布文章先转草稿后才能编辑，归档文章只读。CMS 不实现第二套 Markdown 最终预览。

### CMS 分类与标签

列表提供搜索、真实使用数量与清晰的空/无结果状态；新建和编辑放入抽屉，使移动端操作后立即获得焦点。删除确认必须显示受影响文章数，分类与标签仍遵守各自数据库外键行为。

### CMS 媒体

上传区提供拖放/选择、格式大小限制、替代文本和上传前预览；素材区提供搜索、格式筛选、网格/列表切换、详情与复制公开 URL。素材详情必须说明图片上传后即公开，以及替代文本修改不会回写已经插入 Markdown 的文本。V1 不提供删除、覆盖或重命名。

### CMS 账户

身份区显示当前 owner 邮箱、角色和会话信息；安全区独立承载密码修改，成功与错误使用不同语义反馈。账户页不出现用户列表、邀请或角色配置。
...
- CMS 所有数据页面分别验证 skeleton、保留数据刷新、错误重试、数据库为空和筛选无结果；这些状态不能同时出现。
- 列表筛选写入 URL 并可恢复；移动端行操作不依赖 hover，抽屉关闭后恢复触发按钮焦点。
...
4. 视觉修改完成后，至少检查首页、文章页、CMS 登录、概览、文章列表、编辑器、媒体与账户场景。
