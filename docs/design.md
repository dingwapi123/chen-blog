...
- Markdown/Comark 正式渲染使用 Nuxt Comark；CMS 编辑器只负责输入，最终草稿预览回到 Nuxt 前台。
...
CMS 使用项目自研的精简 Shell 与写作布局，不套用通用后台模板。Element Plus 负责表单、表格、抽屉、上传和反馈等交互原语；品牌层级、工作区结构、主题 token 与响应式布局仍由本设计系统控制。
