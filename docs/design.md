...
公开端不额外引入第三种正文字体。CMS V1 只编辑 Markdown/Comark 源码，以 UI 字体为主；已发布文章的正式阅读排版由公开端负责。
...
- Markdown/Comark 正式渲染使用 Nuxt Comark；CMS V1 编辑器只负责源码输入和安全校验，不提供草稿最终预览。后续预览必须在受 owner 保护且 `noindex` 的 Nuxt 页面实现，不能在 CMS 中复制渲染器。
...
CMS 使用项目自研的精简 Shell 与写作布局，不套用通用后台模板。Element Plus 负责表单、表格、抽屉、上传和反馈等交互原语；品牌层级、工作区结构、主题 token 与响应式布局仍由本设计系统控制。CMS 不使用 Tailwind；当前没有图表或多人协作界面，因此不引入 ECharts、`vue-echarts` 或 Yjs。

公开端与 CMS 的功能图标统一来自 `@lucide/vue`。图标只辅助识别操作，不替代可见文本或 `aria-label`，也不作为装饰性第二强调色。
...
使用窄阅读列与大标题。文章头部包含分类、日期、阅读时长、摘要和更新时间；正文优先保持长时间阅读舒适度。正文至少有两个 h2–h4 标题时，在正文前展示紧凑目录，使用缩进表达层级并保留 `2.5rem` 点击目标；不增加占据阅读宽度的常驻侧栏。页尾标签、上一篇/下一篇与返回列表使用留白组织。
