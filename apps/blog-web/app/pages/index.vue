...
<style scoped>
.home-hero { position: relative; display: grid; min-height: min(45rem, calc(100svh - 4.45rem)); overflow: hidden; border-bottom: 1px solid var(--border); }
.home-hero__texture { position: absolute; inset: -22%; opacity: 0.82; background-image: linear-gradient(var(--border) 1px, transparent 1px), linear-gradient(90deg, var(--border) 1px, transparent 1px), radial-gradient(circle at 52% 45%, var(--accent-soft), transparent 30rem); background-size: 3.5rem 3.5rem, 3.5rem 3.5rem, auto; mask-image: radial-gradient(circle at 60% 50%, black, transparent 65%); transform: rotate(-8deg); }
...
.home-hero__intro { max-width: 34rem; margin: var(--space-lg) 0 0; color: var(--text-muted); font-size: clamp(1.05rem, 1rem + 0.35vw, 1.25rem); }
.home-hero__actions { display: flex; flex-wrap: wrap; gap: var(--space-md); margin-top: var(--space-xl); }
.primary-action, .secondary-action { display: inline-flex; align-items: center; gap: var(--space-sm); padding: 0.72rem 1rem; border-radius: 0.55rem; font-size: 0.92rem; font-weight: 700; }
.primary-action { color: var(--bg-elevated); background: var(--accent); }
.primary-action:hover { background: var(--accent-hover); }
.secondary-action { border: 1px solid var(--border-strong); color: var(--text); }
...
.home-topics { display: grid; grid-template-columns: minmax(0, 0.7fr) minmax(0, 1.3fr); gap: var(--space-2xl); border-top: 1px solid var(--border); }
.topic-grid { display: grid; border-top: 1px solid var(--border); }
.topic-link { position: relative; display: grid; grid-template-columns: 1fr auto; gap: var(--space-xs) var(--space-md); padding-block: var(--space-lg); border-bottom: 1px solid var(--border); }
.topic-link__name { font-size: 1.2rem; font-weight: 700; letter-spacing: -0.03em; }
.topic-link__description { grid-column: 1; color: var(--text-muted); font-size: 0.92rem; }
.topic-link svg { grid-column: 2; grid-row: 1 / span 2; color: var(--text-faint); transition: transform 160ms ease, color 160ms ease; }
.topic-link:hover { color: var(--accent); }.topic-link:hover svg { color: var(--accent); transform: translate(0.14rem, -0.14rem); }
.home-about { border-top: 1px solid var(--border); }.home-about__content { display: grid; grid-template-columns: minmax(0, 0.7fr) minmax(0, 1.3fr); gap: var(--space-2xl); }.home-about p { max-width: 40rem; margin: 0; color: var(--text-muted); font-size: 1.15rem; }
