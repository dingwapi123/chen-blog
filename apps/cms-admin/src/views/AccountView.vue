...
<template><AdminShell><p class="eyebrow">account</p><h1 class="page-title">账户</h1><section class="account"><p><span>当前管理员</span>{{user?.email}}</p><form @submit.prevent="submit"><h2>修改密码</h2><div class="field"><label for="password">新密码</label><input id="password" v-model="password" class="input" minlength="8" required type="password"></div><div class="field"><label for="confirm">确认新密码</label><input id="confirm" v-model="confirmPassword" class="input" minlength="8" required type="password"></div><p v-if="message" class="hint">{{message}}</p><button class="button button--primary" :disabled="saving" type="submit">{{saving?'正在保存…':'更新密码'}}</button></form></section></AdminShell></template>
<style scoped>.account{display:grid;gap:2rem;max-width:28rem;margin-top:1.5rem}.account>p{display:grid;gap:.25rem;padding-bottom:1rem;border-bottom:1px solid var(--border)}.account>p span{color:var(--text-muted);font-size:.8rem}form{display:grid;gap:1rem}form h2{margin:0;font-size:1.1rem}</style>
