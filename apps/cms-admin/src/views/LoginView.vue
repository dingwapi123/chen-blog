  ...
  <main class="login-page">
    <section class="login-panel" aria-labelledby="login-title">
      ...
      <ElForm label-position="top" @submit.prevent="submit">
        ...
        <ElAlert v-if="errorMessage" :closable="false" :title="errorMessage" type="error" show-icon />
...
.login-panel { width: min(100%, 25rem); padding: 2rem; border: 1px solid var(--border); border-radius: 0.75rem; background: var(--bg-elevated); }
.login-panel h1 { margin: 0.35rem 0 0; font-size: 2rem; letter-spacing: -0.05em; }
.intro { margin: 0.45rem 0 1.75rem; color: var(--text-muted); }
