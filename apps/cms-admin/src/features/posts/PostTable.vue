...
function statusType(status: AdminPost['status']) {
...
<template>
  <ElTable v-if="posts.length || loading" v-loading="loading" :data="posts" class="post-table" table-layout="fixed">
    ...
    <ElTableColumn align="right" label="操作" width="132">
      <template #default="{ row }">
        <ElButton v-if="row.status === 'draft'" :loading="publishingId === row.id" link title="发布" type="primary" @click="emit('publish', row.id)">
          <Send :size="15" /><span class="sr-only">发布</span>
        </ElButton>
        <ElButton :loading="archivingId === row.id" link title="软删除" type="danger" @click="emit('archive', row.id)">
...
<style scoped>
.post-table { width: 100%; --el-table-header-bg-color: var(--bg-soft); --el-table-tr-bg-color: var(--bg-elevated); --el-table-row-hover-bg-color: var(--accent-soft); }
.post-title { display: grid; gap: 0.2rem; min-width: 0; color: var(--text); }
.post-title strong { font-size: 0.98rem; }
.post-title span { overflow: hidden; color: var(--text-muted); font-size: 0.82rem; text-overflow: ellipsis; white-space: nowrap; }
.post-title:hover strong { color: var(--accent); }
.updated-at { color: var(--text-muted); font-size: 0.82rem; }
